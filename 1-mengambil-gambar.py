...
while True:
    ...
    if (retR == True) & (retL == True):
        ...
        cv2.drawChessboardCorners(grayR,(7,7),corners2R,retR)
        cv2.drawChessboardCorners(grayL,(7,7),corners2L,retL)
        ...
        if cv2.waitKey(0) & 0xFF == ord('s'):   # Push "s" to save the images and "c" if you don't want to
            ...
            cv2.imwrite('chess-image/chessboard-R'+str_id_image+'.png',frameR) # Save the image in the file where this Programm is located
            cv2.imwrite('chess-image/chessboard-L'+str_id_image+'.png',frameL)
            id_image=id_image+1
        else:
